...
public class EditMenuServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String menuDescription = request.getParameter("menu-description");

        if (id != null) {
            int idEdit = Integer.parseInt(id);
            if (MenuDao.editMenu(idEdit, menuName, menuDescription) != 0) {
                response.setContentType("text/html;charset=UTF-8");
                response.getWriter().print("<script>alert('Update successfully !!!'); window.location.href='./menu-data';</script>");
            } else {
                response.setContentType("text/html;charset=UTF-8");
